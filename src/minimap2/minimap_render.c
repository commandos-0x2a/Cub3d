...
/*   Updated: 2025/10/12 03:19:43 by yaltayeh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_player *player, t_grid *grid)
{
	...
	for (int y = 0; y < minimap->height; y++)
	{
		for (int x = 0; x < minimap->width; x++)
		{
			...
			// // rotation matrix
			// draw_x = (draw_x * cosf(minimap->r)) - (draw_y * sinf(minimap->r));
			// draw_y = (draw_x * sinf(minimap->r)) + (draw_y * cosf(minimap->r));
			...
			if (point.x < 0 || point.x > (int)frame->width 
				|| point.y < 0 || point.y > (int)frame->height)
				continue ;
			map_x = player->pos.x - (1.f * x - minimap->width / 2.f) / DRAWING_SCALE;
			map_y = player->pos.y - (1.f * y - minimap->height / 2.f) / DRAWING_SCALE;
			if (map_x < 0 || map_x >= (int)grid->w
				|| map_y < 0 || map_y >= (int)grid->h)
				color = 0x0 ;
